...
class Index extends Component
{
    ...
    public function delete()
    {
        if ($this->deleteId) {
            ...
            if ($arsip) {
                try {
                    // https://projectyai.com/ - https://github.com/yogi-maulana-dev/
                    ...
                    session()->flash(
                        'success',
                        'Arsip "' . $arsip->judul . '" berhasil dihapus.'
                    );
                } catch (\Exception $e) {
                    session()->flash(
                        'error',
                        'Gagal menghapus arsip: ' . $e->getMessage()
                    );
    ...
    public function render()
    {
        ...
        return view('livewire.admin-arsip.index', [
            ...
            'prodiList' => $this->selectedFakultas
                ? Prodi::where('fakultas_id', $this->selectedFakultas)
                    ->orderBy('nama_prodi')
                    ->get()
                : collect(),
